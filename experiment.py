...
def relative_deviation(approx: int, optimum: int) -> float:
    if optimum == 0:
        return 0.0
    return (optimum - approx) / optimum * 100.0


def run_experiment(
    instances: list[KnapsackInstance],
    ga_cfg: GAConfig,
    ega1_mode: PairingMode = "negative_associative_fitness",
    ...
    for instance in instances:
        ...
        ega1_cfg = GAConfig(
            **{**ga_cfg.__dict__,
               ...
               "random_seed": (ga_cfg.random_seed or 0) + instance.task_id}
        )

        ega2_cfg = GAConfig(
            **{**ga_cfg.__dict__,
               "pairing_mode": ega2_mode,
               "random_seed": (ga_cfg.random_seed or 0) + 1000 + instance.task_id}
        )
...
def format_report(
    ...
    for r in rows:
        lines.append(
            f"{r.task_id:<8}{r.f_opt:<8}{r.ega1_value:<8}"
            f"{r.ega1_delta_percent:<10.2f}"
            f"{r.ega2_value:<8}"
            f"{r.ega2_delta_percent:<10.2f}"
        )
