...
def repair(instance: KnapsackInstance, chromosome: list[int]) -> list[int]:
    """Remove items with smallest v_i / w_i until feasible."""
    repaired = chromosome[:]
...
def create_initial_population(instance: KnapsackInstance, cfg: GAConfig, rng: random.Random) -> list[list[int]]:
...
def select_partner(
    ...
    if mode == "inbreeding":
        return min(mating_pool, key=lambda c: hamming_distance(parent, c))[:]
    if mode == "outbreeding":
        return max(mating_pool, key=lambda c: hamming_distance(parent, c))[:]

    paired = list(zip(mating_pool, fitnesses))
    if mode == "positive_associative":
        return max(paired, key=lambda pair: pair[1])[0][:]
    if mode == "negative_associative_fitness":
        return max(paired, key=lambda pair: abs(parent_fitness - pair[1]))[0][:]

    raise ValueError(f"Unknown pairing mode: {mode}")


def solve_ga(instance: KnapsackInstance, cfg: GAConfig, pairing_mode: PairingMode | None = None) -> KnapsackSolution:
    rng = random.Random(cfg.random_seed)
    population = create_initial_population(instance, cfg, rng)
    active_mode = pairing_mode or cfg.pairing_mode
    allowed_modes = {"random", "inbreeding", "outbreeding", "positive_associative", "negative_associative_fitness"}
    if active_mode not in allowed_modes:
        raise ValueError(f"Unsupported pairing mode in config: {active_mode}")
    ...
    for _ in range(cfg.generations):
        ...
        while len(offspring) < cfg.offspring_size:
            ...
            parent2 = select_partner(parent1, parent1_fitness, population, fitnesses, active_mode, rng)
    ...
    return KnapsackSolution(chromosome=best, total_weight=total_weight, total_value=total_value)
